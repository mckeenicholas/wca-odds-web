...
const processComps = async () => {
    ...
    const parse = await Promise.all(years.map(async year => {
        const res = await fetch(`https://raw.githubusercontent.com/robiningelbrecht/wca-rest-api/master/api/competitions/${year}.json`)
        const json = await res.json()

        const competitions = json.items.map((item: { id: any; date: { from: string | number | Date; }; }) => {
            const id = item.id;
...
const getPersonData = async (persons: string[], event: string) => {
    const parse = await Promise.all(persons.map(async person => {
        const res = await fetch(`https://raw.githubusercontent.com/robiningelbrecht/wca-rest-api/master/api/persons/${person}.json`)
        const json = await res.json()
...
const joinData = async (persons: string[], event: string, monthCutoff: number) => {
...
const testFn = async () => {
    const persons = ["2016LINB01","2017YEHM01","2017WURY01","2017KWAN05","2017WANS05","2017SARE03","2019LUYI03","2015RAME02","2022BHAV01","2019HUAN05","2016PATT02","2015ZHEN17","2023LOPE21","2023ZHAO16","2015WONG02","2022WANG23"];
    ...
    const results = await run_odds_simulation(persons, "333", monthCutoff, 100000);
